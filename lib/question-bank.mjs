...
export const COLUMNS = ["ID", "Question", "Axis", "Tag", "Level", "Round", "Source", "Status", "Asked", "Last"];
 ...
 *
 * `skipped` only ever covers rows found AFTER a recognized header, so it is not
 * on its own enough to tell a caller that nothing was lost: a header with a
 * typo in its `ID` or `Question` cell is not recognized at all, and the whole
 * table is then invisible with `skipped` empty. `header` (empty when none was
 * recognized) and `tableLines` (how many table-shaped lines the file held) are
 * returned so a caller that is about to WRITE can tell "this bank is empty"
 * apart from "this bank's header did not parse" — see question-bank.mjs's
 * migrate command, which refuses on the second.
 *
 * @param {string} markdown
 * @returns {{questions: object[], skipped: number[], header: string[], tableLines: number}}
 */
export function parseQuestionBank(markdown) {
  ...
  return { questions, skipped, header: header ?? [], tableLines };
}
