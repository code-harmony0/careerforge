...
11. Select top 3-4 most relevant projects for the job. **First, drop the duplicates**: if a Projects entry describes the same underlying deliverable as an Experience bullet (same employer/product, e.g. a client project already named under that employer's role), it is a duplicate, not a second proof point. Don't render both. Fold whichever concrete detail the Experience bullet is missing (a metric, a store link, a scope note) into that Experience bullet instead — the same merge-not-duplicate treatment as a manual CV edit — and only pull an *unrelated* project into the Projects section. If `cv.md` carries an Awards / Honors section, populate `awards[]` with the entries that support this role — for an early-career candidate a contest medal or dean's list often outranks a thin project. Omit the key when there is nothing to list and the section disappears entirely; never invent an award to fill it
...
- No nested tables
- Distributed JD keywords: Summary (top 5), first bullet of the 1-2 most JD-relevant roles, Skills section. Don't force keywords into an older/junior role's bullet just to hit this rule — padding a 2021 entry-level bullet with a current senior JD's vocabulary reads as inflation, not relevance; let unrelated older roles stand on their own truthful bullets
